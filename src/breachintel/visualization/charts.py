...
def create_overview_chart(df: pd.DataFrame) -> go.Figure:
    """Monthly breach count with 12‑month moving average."""
    monthly = (
        df.copy()
        .set_index("date")
        .resample("M")
    ...
    monthly["ma_12"] = monthly["breach_count"].rolling(window=12, min_periods=1).mean()

    fig = go.Figure()

    fig.add_bar(
        x=monthly.index,
        y=monthly["breach_count"],
...
def create_breach_type_area(df: pd.DataFrame) -> go.Figure:
    """Stacked area chart of breach types over years."""
    ...
    fig = px.area(
        ...
        x="year",
        color="breach_type",
        title="Evolution of Breach Types",
    )
